...
import { X, Upload, Loader2, Crop as CropIcon, ImagePlus, Trash2 } from 'lucide-react';
...
export default function EditHighlightModal({ highlight, onClose }: EditHighlightModalProps) {
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string>(highlight.imageUrl);
  const [label, setLabel] = useState(highlight.label);
  const [mediaFiles, setMediaFiles] = useState<File[]>([]);
  const [mediaPreviews, setMediaPreviews] = useState<{url: string, type: string, isExisting?: boolean}[]>(
    (highlight.mediaUrls || []).map(url => ({
      url,
  ...
  const handleMediaChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const validFiles = files.filter((file: File) => file.size <= 50 * 1024 * 1024);
  ...
  const handleCancelCrop = () => {
    setIsCropping(false);
    setImageFile(null);
    setImagePreview(highlight.imageUrl); // Revert to original
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let coverUrl = highlight.imageUrl;
      if (imageFile) {
        coverUrl = await uploadToCloudinary(imageFile);
      }
      
      // Upload new media
      const newMediaUrls = await Promise.all(mediaFiles.map(file => uploadToCloudinary(file)));
      
      // Combine existing URLs (that weren't removed) with new URLs
      const finalMediaUrls = [
        ...mediaPreviews.filter(p => p.isExisting).map(p => p.url),
        ...newMediaUrls
      ];
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
        ...
        >
          <div className="flex justify-between items-center p-4 border-b border-zinc-100">
            <h2 className="text-lg font-black text-zinc-900 tracking-tight">Edit Highlight</h2>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center px-1">
                <label className="text-[10px] uppercase tracking-widest font-black text-zinc-400">Media ({mediaPreviews.length})</label>
                <button
                  type="button"
